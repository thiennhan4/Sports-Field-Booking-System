...
export const OwnerLayout = () => {
  const { user, isOwner, switchRole } = useAuth();
    ...
    return (
      <div className="min-h-screen bg-[#0b0f19] flex items-center justify-center p-4">
        <div className="glass-card max-w-md w-full p-8 text-center rounded-3xl animate-fade-in">
          <div className="mx-auto w-16 h-16 bg-purple-500/10 text-purple-400 rounded-2xl flex items-center justify-center mb-6">
            <ShieldAlert className="w-8 h-8" />
          </div>
          <h2 className="text-xl font-bold mb-2 text-white">Yêu cầu quyền Chủ Sân</h2>
          <p className="text-gray-400 text-sm mb-6">
            Khu vực này chỉ dành riêng cho Chủ sân (Owner) để quản lý cơ sở vật chất và lịch đặt.
          </p>
          <div className="flex flex-col gap-3">
            <button
              onClick={() => switchRole("OWNER")}
              className="w-full bg-purple-600 hover:bg-purple-700 text-white font-medium py-2 px-4 rounded-xl transition-all"
            >
              Chuyển sang vai Chủ Sân
            </button>
            <button
              onClick={() => navigate("/")}
