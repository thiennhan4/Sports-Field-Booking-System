...
public class BookingService : IBookingService
{
    ...
    public async Task<IEnumerable<BookingResponseDto>> GetOwnerBookingsAsync(Guid ownerId)
    {
    ...
    public async Task UpdateStatusAsync(Guid userId, Guid bookingId, BookingStatus status)
    {
        var booking = await _bookingRepository.GetByIdAsync(bookingId);
        if (booking == null)
            throw new AppException("Đơn đặt sân không tồn tại.", 404);
        ...
        // If user is Owner, check if court venue belongs to them
        if (user.Role == UserRole.Owner)
        {
            var court = await _courtRepository.GetByIdAsync(booking.CourtId);
            if (court == null || court.Venue.OwnerId != userId)
                throw new AppException("Bạn không có quyền cập nhật đơn đặt sân này.", 403);
        }
