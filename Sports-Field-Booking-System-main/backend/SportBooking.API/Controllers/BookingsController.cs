...
public class BookingsController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<IEnumerable<BookingResponseDto>>>> GetAllBookings()
    {
        ...
        return Ok(ApiResponse<IEnumerable<BookingResponseDto>>.Ok(result, "Danh sách toàn bộ đơn đặt sân."));
    }

    [HttpGet("bookings/owner")]
    [Authorize(Roles = "Admin,Owner")]
    public async Task<ActionResult<ApiResponse<IEnumerable<BookingResponseDto>>>> GetOwnerBookings()
    {
        var userId = Guid.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? Guid.Empty.ToString());
        var result = await _bookingService.GetOwnerBookingsAsync(userId);
        return Ok(ApiResponse<IEnumerable<BookingResponseDto>>.Ok(result, "Danh sách đơn đặt sân của cơ sở."));
    }

    [HttpPut("bookings/{id}/status")]
    [Authorize(Roles = "Admin,Owner")]
    public async Task<ActionResult<ApiResponse<object>>> UpdateStatus(Guid id, [FromQuery] BookingStatus status)
    {
        var userId = Guid.Parse(User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? Guid.Empty.ToString());
        await _bookingService.UpdateStatusAsync(userId, id, status);
        return Ok(ApiResponse<object>.Ok(null!, "Cập nhật trạng thái đơn đặt sân thành công."));
    }
